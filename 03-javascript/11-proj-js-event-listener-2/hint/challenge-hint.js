...
document.onkeyup = function (event) {
  ...
  if (alphabet.indexOf(userPress) !== -1) {
    ...
    document.getElementById("user-key").textContent = userPress;
    // console.log("User press:" + userPress);

    // also store the key into the array we declared at the top of the file
    arrayUserHistory.push(userPress);


    // finally set the text value of the key press history div to the array
    document.getElementById("user-history").textContent = arrayUserHistory;
  } else {
    ...
    document.getElementById("user-key").textContent = ("Press a key that's a letter within our alphabet");
  }
}
